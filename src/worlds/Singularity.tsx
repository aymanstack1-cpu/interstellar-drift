  ...
  return (
    <group>
      ...
      <mesh ref={icosaRef} position={[0, 0, 0]}>
        ...
        <meshStandardMaterial color="#ffffff" emissive="#444444" emissiveIntensity={0.3} wireframe roughness={0.4} metalness={0.6} />
      </mesh>
      <mesh ref={torusRef} position={[0, 0, 0]}>
        <torusKnotGeometry args={[2, 0.4, 64, 8]} />
        <meshStandardMaterial color="#888888" emissive="#333333" emissiveIntensity={0.2} wireframe roughness={0.5} metalness={0.3} transparent opacity={0.6} />
      </mesh>
    </group>
  );
}
...
function DistantParticles() {
  return (
    <points>
      <bufferGeometry>
        <bufferAttribute attach="attributes-position" count={800} array={particlePos} itemSize={3} />
      </bufferGeometry>
      <pointsMaterial color="#555555" size={0.03} transparent opacity={0.4} blending={THREE.AdditiveBlending} sizeAttenuation depthWrite={false} />
    </points>
  );
}
